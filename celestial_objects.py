...
class CelestialObject:
    ...
    @ti.kernel
    def GetMass(self) -> ti.f32:
        return self.m[None]

    @ti.kernel
    def SetMass(self, m: ti.f32):
        self.m[None] = m
    ...
    def computeForce(self, stars: ti.template()):
        for i in range(self.n):
            ...
            if (self.n == 1):
                print(f'i={i}, force={self.force[i][0]}, {self.force[i][1]}')
    @ti.kernel
    def update(self, h: ti.f32):
        ...
        if (self.n == 1):
            print(f'velocity={self.vel[0][0]}, {self.vel[0][1]}')
...
@ti.data_oriented
class SuperStar(CelestialObject):
    def __init__(self, N, mass) -> None:
        super().__init__(N, mass)
        pass
    ...
    @staticmethod
    @ti.func
    def generateThetaAndR(i, j):
        r = ti.random() / 10  # r \in (0, 0.1)
